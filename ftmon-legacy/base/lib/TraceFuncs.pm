...
sub new {
  ...
  if (($max_trace_depth == -1) || ($depth <= $max_trace_depth)) {
    ...
    $self->{'call'} =~ s/::(\w*)\(/::$1\(\n          /g;
    print $trace_file "\n", 
          $space x ($depth - 1), "+-> ", $self->{'call'};

    print $trace_file $space x ($depth - 1), "{\n";
  }
...
sub DESTROY {
  ...
  if (($max_trace_depth == -1) || ($self->{'depth'} <= $max_trace_depth)) {
    ...
    print $trace_file $space x ($self->{'depth'} - 1), "}\n";
    shift @last_depth;
  }
  $depth--;
}
...
TraceFuncs - trace function calls as they happen.
...
TraceFuncs provides utilities to trace the execution of a
