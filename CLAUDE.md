...
Releases: a `v*` tag gates on `ftmon.__version__` matching the tag — bump the
version in **both** `pyproject.toml` and `src/ftmon/__init__.py`.
...
- `.ai/skills/` holds canonical, portable contribution workflows. Read the
  ...
  locations are personal/ignored links, not independently edited copies
  (AS-01/04).
- `extra-monitors/` and `exchange/`: third-party executables remain separately
  installed; recipes and site builds never grant command authority (XR-*/AS-02).
- `tools/` is linted like source. Its outputs — `soak/`, `tuning/evidence/` —
