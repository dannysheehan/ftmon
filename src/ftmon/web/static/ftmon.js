...
document.querySelectorAll('[data-trend-select]').forEach(select=>select.addEventListener('change',()=>{const current=new URLSearchParams(window.location.search),params=new URLSearchParams({range:current.get('range')||'24h'});window.location.assign(`/trends/${select.value}?${params}`)}));
...
document.querySelectorAll('[data-metric-monitor]').forEach(select=>select.addEventListener('change',()=>{const form=select.form,params=new URLSearchParams(new FormData(form));params.delete('entity');params.delete('metric');params.delete('group');window.location.assign(`/metrics?${params}`)}));
...
/* Baselines keep their native five-minute timestamps. The server partitions
   exact contiguous runs so this bounded plugin cannot claim evidence across a
   missing/pruned bucket or align points onto unrelated raw/hourly timestamps. */
const baselinePlugin=(runs)=>({hooks:{draw:[u=>{if(!runs.length)return;const ctx=u.ctx;ctx.save();ctx.strokeStyle='#6f42c1';ctx.fillStyle='#6f42c1';ctx.lineWidth=2;ctx.setLineDash([7,5]);runs.forEach(run=>{if(run.length>1){ctx.beginPath();run.forEach((point,index)=>{const x=u.valToPos(point[0],'x',true),y=u.valToPos(point[1],'y',true);if(index)ctx.lineTo(x,y);else ctx.moveTo(x,y)});ctx.stroke()}run.forEach(point=>{const x=u.valToPos(point[0],'x',true),y=u.valToPos(point[1],'y',true);ctx.beginPath();ctx.arc(x,y,2.5,0,Math.PI*2);ctx.fill()})});ctx.restore()}]}});
