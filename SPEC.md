...
Status: **DRAFT v0.33** — v0.33 makes Windows Event Log channel selection
and per-channel subscribe-time filtering configurable (MD-13, DM-19,
SA-10): a `source = "events"` monitor's `[source_options]` can now declare
`channels` (path + optional XPath query, unioned across every loaded event
monitor) instead of the hardcoded System/Application default, and a bad
channel name or malformed query is isolated to that one channel instead of
aborting the whole subscription pass. Also closes a real doc/code gap:
DM-09's `store_min_severity` override was documented but had no schema
branch to actually accept it. v0.32 replaces the `windowsdesktop` placeholder
...
**v0.33 (2026-07-27)** — Windows Event Log channel selection and
per-channel subscribe-time filtering (MD-13, DM-19, SA-10). Grew out of
analyzing two days of real desktop incident data: the only channels
available (hardcoded System/Application) miss the events people actually
want for security-relevant monitoring, and Windows' Event Log query engine
(the XPath-subset language shared by `EvtQuery`/`EvtSubscribe`/`wevtutil`/
...
MD-13 fixes that for both platforms.
