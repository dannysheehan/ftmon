...
could not tell them apart (RB-02, issue #143). v0.53 completes RB-02's per-source duration clause
...
markers, so sharing a trend between groups cannot substitute one incident's
evidence for another's (UI-12/UI-13, issue #106). v0.51 bounds SA-05's previously unbounded promotion
...
transition events (SA-05, DM-16, RB-02, issue #103). v0.50 attributes retained catalog entities and
series to their owning monitor in `ftmon doctor` and `/self`, using bounded
catalog-only reads and labels that keep presence distinct from current
persisted-selection pressure (CL-05, UI-02, DM-16, issue #103). v0.49 lets
MD-09 reap expire the hourly rollups of
long-gone entities instead of only waiting for them: a catalog cannot shrink
faster than its longest retention window, and on a live desktop 7,923 dead
entities were pinned by `rollup1h` alone, none reapable, with the 90 d window
still 61 days from first firing (DM-04, MD-09, DM-16, issue #103).
v0.48 extends DM-04's durable/process retention
...
- **DM-04** Raw samples are kept **48 h**. 5-minute rollups `(avg, min, max, last, count)` are kept **30 d** for *durable* series and **7 d** for process-sourced series (v0.47 amendment, issue #102: the same churn argument the hourly tier settled in v0.3, applied to a tier twelve times denser and, in practice, the largest table in the database; no rule reads that far back either way, since CA-04 caps expression windows at 6 h and CA-05 baselines step incrementally as rollups are produced rather than re-reading history). 1-hour rollups are kept **400 d** for *durable* series (system, disk, self, and watchlist-synthetic entities) and **90 d** for process-sourced series (v0.3 amendment: the capacity worksheet shows process-entity churn makes 400 d hourly retention for all series infeasible within DM-05). Hourly rollups of a **process-sourced** series whose entity has been **continuously `gone` for longer than the process 5-minute window (7 d)** are expired regardless of the 90 d figure (v0.49 amendment, issue #103): the 90 d window assumes a series whose entity still exists, and for a process that exited a week ago it is the sole remaining reason its catalog rows cannot be reclaimed. The threshold is the 5-minute process window rather than a chosen constant, because that is the shortest point at which no *other* DM-04 window still holds data for that series — raw expired at 48 h and 5-minute at 7 d — so expiry here never cuts inside a window this requirement separately promises. That reasoning is **why the rule is confined to process-sourced series**: a durable series keeps 5-minute data for 30 d and hourly for 400 d, so the same cut would destroy history still promised, and durable entities that legitimately go `gone` — an unmounted filesystem, a removed interface — keep their full window. Live entities are unaffected at any age: durability describes the source kind, not liveness, and a long-running process keeps its full 90 d. Rollup jobs run in the daemon, incrementally, never more than 1 s of work per cycle.
...
- **DM-16** The design document MUST include a capacity worksheet deriving RB-01/DM-05 feasibility from stated assumptions — max tracked entities (budget: 400 persisted), metrics per entity (≤ 10), sample width in bytes, rows/day at 60 s intervals, event rates, ring-buffer memory (CA-04) — and the worksheet MUST distinguish host-wide planning estimates from enforcement limits. Static quantities are rejected at definition validation; runtime-only quantities are bounded and reported where they are admitted. (v0.51 amendment, issue #103.) In particular, whether a promotion expression matches three processes or thousands cannot be known at load time, so SA-05 applies a chosen per-monitor concentration guardrail during sampling rather than claiming validation can predict host cardinality. This guardrail complements but does not allocate the worksheet's host-wide 400 persisted-entity budget. (v0.46 amendment, issue #104.) Because the budget counts **persisted** entities, pressure against it MUST be measured as those for which durable history is currently being written — not as entities merely present. A process the pipeline samples but does not select is running, not persisted: under SA-05 track-all every sampled entity is marked seen, so counting presence overstates storage pressure by the ratio of sampled to selected entities, which on a desktop is roughly an order of magnitude. A count derived from presence MAY still be reported, under a name that says so.
...
Metrics: `cpu_pct`. Rules (group `hog`): warning when `avg(cpu_pct, "5m") > 80` for `confirm_cycles = 5`; error rung at `avg(cpu_pct, "15m") > 90`. Default exempt examples (commented): `matches(name, "^(cc1|rustc|ld|clang|make|cargo|ffmpeg)")`. Glance: maximum five-minute CPU average with only its matching warning parameter; the fifteen-minute error threshold MUST NOT be presented as though it applied to that value. The macOS profile MUST guard both rungs with `cpu_pct_readable == "true"` before reading the metric: known OS permission denial is outside that rule's applicability, while an absent marker or a `"true"` marker without `cpu_pct` still evaluates UNKNOWN and remains diagnosable.
...
processes without `cpu_pct`; both stock hog rungs therefore accumulated
permanent UNKNOWN findings. The sampler now publishes a
...
the shared sample, which includes persistence selection and disappearance
tracking, not merely rule evaluation. On the reference canary that total was
3.5x the entire sampling stage and grew with daemon uptime while sampling did
not, and a single number could not say whether the growth was evaluation over a
stable entity set or catalog pressure against an ageing database. Five fixed
phases now partition it exactly, on the same closed-vocabulary terms as v0.53's
source split: no monitor, rule, or entity dimension, so the DM-16 catalog budget
stays independent of what an operator installs (RB-02, DM-16, TS-03, issue #143).
...
than claiming separately bounded check execution. The Self page computes every
visible counter over one shared post-upgrade span and renders explicit parent
relationships so operators do not double-count child rows. DM-16's worksheet
and diagnostic assumption now include the current 58-series self monitor
...
Trend, an RSS level breach offers both peak RSS and that shared Trend, and a
database breach opens the used-capacity Trend. Every link carries its entity,
24-hour range and originating group; Metrics and Trends visibly identify and
filter markers to that group, including a clear empty-filter state, and discard
the group when switching monitors. This lets `rss-budget` reuse the RSS growth
panels without relabelling `rss-growth` incidents as its evidence
(UI-12/UI-13, issue #106).
...
depends on the future process population, so DM-16 no longer promises that
this runtime quantity can be rejected during definition validation. SA-05 now
admits at most ten promoted entities per monitor as a chosen concentration
guardrail, distinct from the worksheet's host-wide ~10 planning estimate. The
cap affects persistence only: rules still
evaluate for every non-exempt sampled process and top-N remains independently
bounded. Existing promotions keep their slots, new matches are admitted in
stable entity-id order without claiming severity ranking, and one
enter/recovery event names the monitor instead of emitting per-entity or
per-tick noise. Fixed self metrics make current
limited-monitor count and cumulative admission refusals visible to doctor and
`/self`. The definition documentation now states the persistence/evaluation
boundary plainly and warns against promotion expressions over unbounded,
churning classes.

**v0.50 (2026-08-09)** — advances issue #103's attribution work. The catalog
could identify a 152,297-series total but not which definition created it;
diagnosing one monitor holding 101,064 series required hand-written SQL.
Doctor and `/self` now attribute retained catalog entities and series by
monitor through a bounded query over `entities` and `series` only. Presence is
reported separately and is never relabelled as current persisted-selection
pressure: the pipeline publishes that pressure globally, and the retained
catalog cannot reconstruct its per-monitor split. Creation rate, reapability,
and admission caps remain separate work.

**v0.49 (2026-08-09)** — addresses issue #103. #102 stopped the desktop canary
degrading, but the relief is temporary: measured non-expiring growth is
~1.01 MB/day against 24.9 MB of headroom, so DM-05 degradation returns in
three to four weeks, while the window that would relieve it does not fire for
about sixty days.

The cause is that MD-09 reap removes an entity only once *every* tier is
empty, so catalog size is a function of the longest window rather than of any
entity budget. On that host 7,923 entities dead longer than a week were pinned
by `rollup1h` alone — zero by `samples`, zero by `rollup5m` — and none were
reapable. The oldest hourly bucket was 28.6 d old, so the 90 d process window
had never once fired in production.

Reap now expires those hourly rollups rather than waiting for them. The
threshold is DM-04's process 5-minute window (7 d) rather than a chosen
constant: it is the shortest point at which no other DM-04 window still holds
data for the entity, so expiry never cuts inside a window the spec separately
promises. A 30 d threshold was measured and rejected — it would have freed
49 entities on a 28.6-day-old database, which is to say nothing. At 7 d it
frees 13,076 entities and 76,096 series, about half the catalog.

Live entities are untouched at any age. Durability describes the source kind,
not liveness, so shortening the window itself would have truncated history for
long-running processes to solve a problem caused by dead ones; expiring on
gone-duration separates the two. Deletion is bounded by rows and elapsed time
as well as the existing entity cursor, because a bound on entities visited is
not a bound on work done.
...
the worksheet's ≤400/~320 assumptions under process churn even when reap is
